		...
		if (flag) {
			if (creep.room == flag.room) {
				...
				if (enemies.length > 0 || enemyStructures.length > 0) {
					...
					let functionalMeleeAttackBody = meleeAttackBodies.find(
						(b) => b.hits > 0
					);
            ...
            */
					...
					let closestNonSpawnClosest = creep.pos.findClosestByRange(
						closestNonSpawn
					);

					if (
						creep.attack(colsestTarget) == ERR_NOT_IN_RANGE ||
