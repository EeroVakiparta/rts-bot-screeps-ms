...
module.exports.loop = function () {
    ...
    if(!underAttack){
        ...
        var builders1 = _.filter(Game.creeps, (creep) => creep.memory.role == 'builder' && creep.memory.source == 'container' && creep.memory.sourceIndex == 1 && creep.memory.targetFlag == 'flag1' );
        if(builders1.length < 0 && cHarvSpawn2.length > 0 && cHarvSpawn21.length > 0) {
            var newName = 'Builder' + Game.time;
            //console.log('Spawning new builder: ' + newName);
            Game.spawns['Spawn2'].spawnCreep([WORK,WORK,CARRY,MOVE,MOVE,MOVE], newName, // WORK,WORK,WORK,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE
        ...
        if(haulersspawn2.length < 3 && cHarvSpawn2.length > 0 && cHarvSpawn21.length > 0) {
    ...
    /////////////////////////
        ...
        if(buildersUnderFlag.length < 1 && cHarvesters0.length > 0 && cHarvesters1.length > 0) {
        ...
        // --Make sure there is at least 1 builder at all times
        var builders0 = _.filter(Game.creeps, (creep) => creep.memory.role == 'builder' && creep.memory.source == 'sources' && creep.memory.sourceIndex == 0 );
        ...
        // --Make sure there is at least 1 upgrader at all times
        var flagUpgraders0 = _.filter(Game.creeps, (creep) => creep.memory.role == 'upgrader' && creep.memory.targetFlag == 'flag1' && creep.memory.sourceIndex == 0);
        //console.log('Upgrader: ' + upgraders.length);

        if(flagUpgraders0.length < 0 && cHarvesters0.length > 0 && cHarvesters1.length > 0) {
            var newName = 'FlagUpgrader' + Game.time;
            //console.log('Spawning new upgrader: ' + newName);
            Game.spawns['Spawn2'].spawnCreep([WORK,WORK,WORK,WORK,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE], newName,
                {memory: {role: 'upgrader', targetFlag: 'flag1', sourceIndex: 0}});
        }
        
                // Flag Upgrader spawner
        // --Make sure there is at least 1 upgrader at all times
        var flagUpgraders1 = _.filter(Game.creeps, (creep) => creep.memory.role == 'upgrader' && creep.memory.targetFlag == 'flag1' && creep.memory.sourceIndex == 1);
        ...
        if(flagUpgraders1.length < 0 && cHarvesters0.length > 0 && cHarvesters1.length > 0) {
            ...
            //console.log('Spawning new upgrader: ' + newName);
            Game.spawns['Spawn1'].spawnCreep([WORK,WORK,WORK,WORK,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE], newName,
                {memory: {role: 'upgrader', targetFlag: 'flag1', sourceIndex: 1}});
        }
    ...
    if(hostiles.length > army.length && underAttack) {
        ...
        Game.spawns['Spawn1'].spawnCreep([TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,TOUGH,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,ATTACK,ATTACK,ATTACK], newName,
    ...
    // It seems like equal battles are still lost before I make smarter army. So until then +1
    if(hostiles2.length + 1 > army2.length) {
        var newName = 'Army' + Game.time;
        //console.log('Under attack ! Spawning new armycreep: ' + newName);
...
// Game.spawns['Spawn1'].spawnCreep( [CLAIM, MOVE], 'Claimer1', { memory: { role: 'claim', targetFlag: 'flag1'}} );
    if(claim.length < 0 && Game.gcl.level > 2 && cHarvesters0.length > 0 && cHarvesters1.length > 0) {
        var newName = 'Claim' + Game.time;
        
        Game.spawns['Spawn1'].spawnCreep([CLAIM,MOVE], newName,
            { memory: { role: 'claim', targetFlag: 'flag1'}});
    }
